...
module.exports.run = (client, message, args) => {
  ...
  if(message.author.id == wl) {
    ...
    var str_filtrer = message.guild.members.cache.filter(member => member.hasPermission("BAN_MEMBERS") && !member.user.bot)
  ...
  //   message.channel.send(`Liste des membres ayant les permissions \`BAN_MEMBERS\` (**${str_filtrer.size}**)`)
    for(let i = 0; i < str_map.length; i += 1995) {
      ...
      const banperm = new Discord.MessageEmbed()
        .addField(`__Liste des membres ayant les permissions de \`BAN_MEMBERS\`__ !`, `${str_content}`, true)
        .setFooter(`Total: ${str_filtrer.size}・Surfinia Protect`)
        .setColor("#6cc7ff")
      message.channel.send({embed: banperm}) 
